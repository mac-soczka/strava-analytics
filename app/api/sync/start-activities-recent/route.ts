...
export async function POST(request: NextRequest) {
  try {
    ...
    const recentWindowDays = Number(url.searchParams.get('days') ?? '30')

    const syncService = new SyncOrchestrationService(user.strava_id)
    const job = await syncService.startActivitiesRecentSync(user.strava_id, recentWindowDays)
    ...
    console.error('Error starting recent activities sync job:', error)
    ...
    return NextResponse.json(
      { error: 'Failed to start recent activities sync job', details: error?.message },
      { status: 500 }
    )
  }
}
