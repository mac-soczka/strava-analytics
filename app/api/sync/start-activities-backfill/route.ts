...
export async function POST(request: NextRequest) {
  try {
    ...
    const job = await syncService.startActivitiesBackfillSync(user.strava_id)
    ...
    console.error('Error starting activities backfill sync job:', error)
      ...
      return NextResponse.json(
        {
          ...
          details: 'Missing enum value "activities_only" in sync_job_type. Apply latest migrations (supabase db push / apply migration).',
        },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Failed to start activities backfill sync job', details: error?.message },
      { status: 500 }
    )
  }
}
